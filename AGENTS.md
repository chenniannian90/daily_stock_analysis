...
- **A股配色规则**：上涨用红色，下跌用绿色。前端所有涨跌颜色统一遵循此规则（与 A 股行情软件一致）。
...
- **远程服务器网络测试**：测试股票新闻、搜索、数据抓取等涉及外部网络接口的功能时，必须在远程服务器 (root@117.72.39.169, /data/daily_stock_analysis) 上执行。因为本地开发机网络环境与生产环境不一致（部分数据源在本地受限但服务器上畅通），禁止以本地网络不通为由判定接口不可用。
- 远程服务器部署方式：`docker compose -f docker/docker-compose.yml exec -T server python <script_or_module>`。同步代码使用 `scp` 或 `rsync` 上传文件后，再通过 ssh 远程执行。

### 1.1 AI 操作准则

> 以下规则适用于本项目的每一次 AI 辅助任务，除非被明确覆盖。

1. **先想再做**：在非 trivial 工作上，先陈述假设和方案再动手。存在歧义时先澄清，不猜测。发现更简单方案时主动反馈。困惑时停下来，明确说不清楚的地方。
2. **简单优先**：只写解决问题所需的最少代码。不做推测性开发，不为单次使用创建抽象，不写半成品实现。问自己：高级工程师会认为这是过度设计吗？
3. **精准改动**：只触达必须改的文件和代码，不顺手清理无关代码、注释或格式，不重构未出问题的部分，保持与现有风格一致。
4. **目标驱动**：定义成功标准，循环迭代直到验证通过。不机械遵循步骤，而是用成功标准驱动独立迭代。
5. **模型只做判断**：AI 模型用于分类、草拟、总结、提取等判断型任务，不用于路由、重试、确定性变换这类代码可以完成的逻辑。
6. **Token 预算不是建议**：每次任务 4000 token，每次会话 30000 token。接近预算时主动总结并开启新会话，不做静默超支。
7. **冲突时选边，不平摊**：两个模式矛盾时，选用更新或更经过验证的那个，解释原因，标记另一个后续清理。不混合矛盾模式。
8. **写前先读**：加代码前，先读相关 exports、调用方、共享工具。表面看起来正交的改动可能隐藏耦合。
9. **测试验证的是意图，不止是行为**：测试必须编码为什么这个行为重要，而不只是描述它做了什么。无法在业务逻辑变更时失败的测试是不对的。
10. **每一步后打检查点**：总结做了什么、验证了什么、还剩什么。无法描述清楚当前状态就不要继续。跟丢时停下来重新梳理。
11. **遵循仓库约定，即使你不同意**：代码仓库内约定优先于个人偏好。如果真心认为某约定有害，提出来讨论，但不要默默分叉。
12. **大声失败**：任何静默跳过的步骤都不能声称已完成。跳过的测试不能声称已通过。默认呈现不确定性，而非隐藏它。
